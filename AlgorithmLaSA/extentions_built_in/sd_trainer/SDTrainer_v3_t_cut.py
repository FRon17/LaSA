...
class SDTrainer(BaseSDTrainProcess):
    ...
    def calculate_loss(
        ...
        # =================【v4 终极版：Style Loss 独立加权流程】=================
        # 🔴 核心修复：让 Base Loss 和 Style Loss 分别走完加权流程，最后再合并
        
        # 1. 准备时间步
        t_float = timesteps.float()
        if t_float.max() > 1.0:
            t_float = t_float / 1000.0
        t_view = t_float.view(-1, 1, 1, 1)
        
        # 2. 时间步截断（只在清晰阶段计算 Style Loss）
        valid_mask = (t_float < 0.5).float().view(-1, 1, 1, 1)  # [B, 1, 1, 1]
        
        style_loss_per_element = torch.zeros_like(loss)  # [B, C, H, W]
        style_loss_value_raw = 0.0
        
        # 3. 计算 Style Loss（仅 2D 图像）
        if len(noise_pred.shape) == 4 and valid_mask.sum() > 0:
            pred_x0 = noisy_latents - (t_view * noise_pred)
            ...
            def get_safe_stats(x):
                mu = x.mean(dim=(2, 3))
                var = torch.var(x, dim=(2, 3), unbiased=False)
                std = torch.sqrt(var + 1e-5)
                return mu, std

            leader_mu, leader_std = get_safe_stats(leader)
            leader_mu, leader_std = leader_mu.detach(), leader_std.detach() 
            
            for i, f in enumerate(followers):
                f_mu, f_std = get_safe_stats(f)
                l_mu = (f_mu - leader_mu).pow(2)
                l_std = (f_std - leader_std).pow(2)
                l_combined = (l_mu + l_std).unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
                
                if i == 0:  # 右上
                    style_loss_per_element[:, :, :h, w:] = l_combined.expand(-1, -1, h, w)
                elif i == 1:  # 左下
                    style_loss_per_element[:, :, h:, :w] = l_combined.expand(-1, -1, h, w)
                elif i == 2:  # 右下
                    style_loss_per_element[:, :, h:, w:] = l_combined.expand(-1, -1, h, w)
            
            style_loss_per_element = style_loss_per_element / 3.0
            style_loss_per_element = style_loss_per_element * valid_mask  # 应用时间步掩码
            
            # 记录原始 Style Loss（未加权）
            valid_count = valid_mask.sum().item()
            if valid_count > 0:
                style_loss_value_raw = (style_loss_per_element.sum() / valid_count).item()
        
        # =================【关键分叉点：Base 和 Style 独立处理】=================
        
        # 4. 保存纯净的 Base Loss（此时还是 [B, C, H, W]）
        base_loss_4d = loss.clone()
        
        # 5. 压缩 Base Loss 成 [B]
        if len(noise_pred.shape) == 5:
            base_loss_1d = base_loss_4d.mean([1, 2, 3, 4])
        else:
            base_loss_1d = base_loss_4d.mean([1, 2, 3])
        
        # 6. 压缩 Style Loss 成 [B]
        if len(noise_pred.shape) == 5:
            style_loss_1d = style_loss_per_element.mean([1, 2, 3, 4])
        else:
            style_loss_1d = style_loss_per_element.mean([1, 2, 3])
        
        # 7. 分别应用样本权重和 SNR Gamma 加权
        try:
            base_loss_1d = base_loss_1d * loss_multiplier.view(-1)
            style_loss_1d = style_loss_1d * loss_multiplier.view(-1)
        except:
            pass
        
        if prior_loss is not None:
            base_loss_1d = base_loss_1d + prior_loss
        
        if not self.train_config.train_turbo:
            if self.train_config.learnable_snr_gos:
                base_loss_1d = apply_learnable_snr_gos(base_loss_1d, timesteps, self.snr_gos)
                style_loss_1d = apply_learnable_snr_gos(style_loss_1d, timesteps, self.snr_gos)
            elif self.train_config.snr_gamma is not None and self.train_config.snr_gamma > 0.000001 and not ignore_snr:
                base_loss_1d = apply_snr_weight(base_loss_1d, timesteps, self.sd.noise_scheduler, self.train_config.snr_gamma, fixed=True)
                style_loss_1d = apply_snr_weight(style_loss_1d, timesteps, self.sd.noise_scheduler, self.train_config.snr_gamma, fixed=True)
            elif self.train_config.min_snr_gamma is not None and self.train_config.min_snr_gamma > 0.000001 and not ignore_snr:
                base_loss_1d = apply_snr_weight(base_loss_1d, timesteps, self.sd.noise_scheduler, self.train_config.min_snr_gamma)
                style_loss_1d = apply_snr_weight(style_loss_1d, timesteps, self.sd.noise_scheduler, self.train_config.min_snr_gamma)
        
        # 8. 融合 Base 和 Style Loss（权重 0.05）
        style_weight = 0.1 / 10000 # 0.05是真的权重，10000是缩放因子
        loss = base_loss_1d.mean() + (style_weight * style_loss_1d.mean())
        
        # 9. 记录日志
        base_loss_value = base_loss_1d.mean().item()
        style_loss_value = style_loss_value_raw / 10000.0  # 记录未加权的原始值
