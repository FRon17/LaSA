...
class SDTrainer(BaseSDTrainProcess):
    ...
    def calculate_loss(
        ...
        # =================【开始插入 Grid 内部一致性控制】=================
        # =================【终极修复版：Grid 内部一致性控制】=================
        # 1. 直接对 noise_pred (Velocity) 进行切分，跳过反推 x_0 的误差放大陷阱
        B, C, H, W = noise_pred.shape
        h, w = H // 2, W // 2
        
        # Leader (左上)
        leader = noise_pred[:, :, :h, :w]
        # Followers (其他三个)
        followers = [
            noise_pred[:, :, :h, w:], 
            noise_pred[:, :, h:, :w], 
        ...
        # 2. 安全的统计量计算函数 (彻底修复 NaN 梯度陷阱)
        def get_safe_stats(x):
            # x: [B, C, H, W] -> mu/std: [B, C]
            mu = x.mean(dim=(2, 3))
            # 必须在 sqrt 内部加上 epsilon，防止方差为 0 时导数爆炸
            var = torch.var(x, dim=(2, 3), unbiased=False)
            std = torch.sqrt(var + 1e-5)
            return mu, std

        # 3. 计算 Leader 目标 (切断梯度，作为锚点)
        ...
        # 5. 融合 Loss (废弃冲突的时间步动态加权，使用固定权重)
        # 因为是对 Latent 级别的 Velocity 算 MSE，数值通常较大，0.01 是一个安全的起点
        style_weight = 0.01 
        ...
        # 最终 Loss
        loss = loss + (style_weight * style_loss_batch)
        
        final_loss_value = loss.mean().item()
        
        # 打印日志 (可选：如果嫌吵可以注释掉)
        # print(f"🔴 Base Loss = {base_loss_value:.6f}, Style Loss = {style_loss_value:.6f}, Final = {final_loss_value:.6f}")
